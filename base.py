...
def train(env, actor, critic, optim, num_episodes, num_actors, num_epochs, num_replay_episodes, eps, loss_calculator, batch_size):
    ...
    try:
        ...
        for i in pbar:
            ...
            obj_func_hist.append(pack_data([sum(reward for _, _, _, reward, _, _, _ in episode) for episode in unprocessed_episodes]))
            ...
            for _ in range(num_epochs):
                ...
                for batch_start in range(0, len(unprocessed_episodes), batch_size):
                    ...
                    states, actions, original_probs, rewards, states_t1, actions_t1, terminated = zip(*unprocessed_episodes[batch_start:batch_end])
                    ...
                    terminated = torch.tensor((~np.array(terminated, dtype=bool)).astype('float')).unsqueeze(1)

                    critic_values = critic(states)
                    critic_values_t1 = critic(states_t1)

                    loss_calculator.update_losses(critic_values, actions, rewards, critic_values_t1, actions_t1, terminated, actor.device)
                    ...
                    ppo_loss = torch.mean(ppo_gradient)# mean to normalize by length. To make length of episode not have influence on how to update models. Also necessary in case of batches
