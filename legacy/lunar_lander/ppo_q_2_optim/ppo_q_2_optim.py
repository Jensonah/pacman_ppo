...
def train(env, actor, critic, optim_actor, optim_critic, num_episodes, num_actors, num_epochs, eps, gamma):
    ...
    try:
        ...
        for i in tqdm(range(num_episodes)):
            ...
            for k in range(num_epochs):
                ...
                for j in range(num_actors):
                    ...
                    critic_values_best_t1 = torch.cat((critic_values_best_t.clone()[1:], torch.zeros(1,1)))
                    ...
                    #critic_loss += torch.mean((critic_values_action_t - rewards_std[j] - gamma*critic_values_action_t1)**2)
                    ...
                    advantage = (gamma*critic_values_action_t1 - (critic_values_action_t - rewards_std[j])).detach()
        ...
        return obj_func_hist, ppo_losses, critic_losses
...
def plot_fancy_loss(data, path, title, y_label):
    ...
    plt.plot(xs, line_200,  color='red',  linestyle='dashed', alpha=0.1)
    plt.plot(xs, line_0,    color='red',  linestyle='dashed', alpha=0.1)

    # Plot range
    plt.fill_between(xs, mins, maxes, color='blue', alpha=0.3)
...
optim_actor = optim.Adam(actor.parameters(), lr=lunar_lander_hyperparameters["lr"])

optim_critic = optim.Adam(critic.parameters(), lr=lunar_lander_hyperparameters["lr"])

obj_func_hist, ppo_loss, critic_loss = train(env,
                              actor,
                              critic,
                              optim_actor,
                              optim_critic,
