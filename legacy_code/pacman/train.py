...
from base import Actor, Critic, collect_episode
    ...
    for _ in tqdm(range(num_episodes)):
        ...
        for k in range(num_epochs):
            ...
            for j in range(num_actors):
                critic_values_t = torch.cat(
                    [critic(get_state(i, frames[j], critic)) for i in range(len(frames[j]))]
                )
...
env = gym.make(
    pacman_hyperparameters["env_name"], render_mode=pacman_hyperparameters["render_mode"]
)
...
actor = Actor(pacman_hyperparameters["device"], dim, pacman_hyperparameters["no_frames"])
actor.to(actor.device)

critic = Critic(pacman_hyperparameters["device"], dim, pacman_hyperparameters["no_frames"])
critic.to(critic.device)

optimizer = optim.Adam(
    list(actor.parameters()) + list(critic.parameters()), lr=pacman_hyperparameters["lr"]
)

obj_func_hist, losses = train(
    env,
    actor,
    critic,
    optimizer,
...
plot_fancy_loss(obj_func_hist, f"{base_path}/rewards.png")

plot_ugly_loss(losses, len(losses), "total_")
